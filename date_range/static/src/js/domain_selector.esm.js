...
patch(DomainSelector.prototype, {
    ...
    async onPropsUpdated(p) {
        // Let super handle tree building (it uses treeProcessor internally in Odoo 19)
        await super.onPropsUpdated.apply(this, arguments);

        // If super could not build a tree (unsupported domain), bail out
        if (!this.tree) {
            return;
        }
        ...
        const rawDomain = domain.toList();

        // Post-process the tree that super already built, adding date range operators
        this.tree = treeFromDomainDateRange(rawDomain, {
