...
class PosCashTransfer(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin']  # Fix: required for <chatter/> in form view
    ...
    from_session_id = fields.Many2one(
        'pos.session', string='From POS Session', required=True, tracking=True)
    to_session_id = fields.Many2one(
        'pos.session', string='To POS Session', required=True, tracking=True)
    ...
    amount = fields.Float(string='Amount', required=True, digits='Account', tracking=True)
