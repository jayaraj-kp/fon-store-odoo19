...
// ═══════════════════════════════════════════════════════════════════════════════
// HELPER: get orders from the model respecting current state.filter
// This mirrors what Odoo 19's TicketScreen does internally, without relying
// on this.activeOrderFilter which may not exist in all versions.
// ═══════════════════════════════════════════════════════════════════════════════

function getBaseOrders(screen) {
    const orderModel = screen.pos.models["pos.order"];
    const filter = screen.state.filter;

    // "SYNCED" = Paid orders tab
    if (filter === "SYNCED") {
        return orderModel.filter((o) => o.finalized && o.uiState?.displayed !== false);
    }

    // "ACTIVE_ORDERS" = active (open) orders tab
    if (filter === "ACTIVE_ORDERS") {
        // Try using activeOrderFilter getter if it exists, else fall back
        if (typeof screen.activeOrderFilter === "function") {
            return orderModel.filter(screen.activeOrderFilter);
        }
        return orderModel.filter((o) => !o.finalized);
    }

    // Any other named filter (e.g. "RECEIPT_NUMBER" custom statuses) —
    // use activeOrderFilter if available, otherwise fall back to all
    if (typeof screen.activeOrderFilter === "function") {
        try {
            return orderModel.filter(screen.activeOrderFilter);
        } catch (_e) {}
    ...
    // Final fallback: return finalized (paid) orders since "Paid" is default
    return orderModel.filter((o) => o.finalized && o.uiState?.displayed !== false);
}
...
patch(TicketScreen.prototype, {
    ...
    async onSearch(search) {
        if (search?.fieldName === "CUSTOM_FILTER") {
            ...
            if (result && result.rules && result.rules.length > 0) {
                this._customFilter = result;
                // Force a re-render by updating state without touching state.filter.
                // We directly mutate state.search to a safe neutral value so the
                // reactive system re-runs getFilteredOrderList() with _customFilter set.
                this.state.search = { fieldName: "RECEIPT_NUMBER", searchTerm: "__CUSTOM_FILTER_ACTIVE__" };
            }
            return;
        }
    ...
    getFilteredOrderList() {
        ...
        // ── Custom filter (highest priority) ─────────────────────────────────
        if (this._customFilter && this._customFilter.rules && this._customFilter.rules.length > 0) {
            // Use getBaseOrders() to correctly respect the Paid/Active tab selection
            let orders = getBaseOrders(this);
        ...
        if (EXACT_FIELDS.has(fieldName) && searchTerm) {
            const orderModel = this.pos.models["pos.order"];
            let orders = this.state.filter === "SYNCED"
                ? orderModel.filter((o) => o.finalized && o.uiState?.displayed !== false)
                : orderModel.filter(this.activeOrderFilter);
            if (this.state.filter && !["ACTIVE_ORDERS", "SYNCED"].includes(this.state.filter)) {
                orders = orders.filter((order) => {
                    const screen = order.getScreenData();
