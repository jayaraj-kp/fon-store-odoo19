...
def post_init_hook(env):
    ...
    cr.execute("""
        ALTER TABLE pos_config
            ...
            ADD COLUMN IF NOT EXISTS pos_address_city_pin  VARCHAR,
            ADD COLUMN IF NOT EXISTS pos_address_phone     VARCHAR,
            ADD COLUMN IF NOT EXISTS pos_customer_prefix   VARCHAR;
    """)
...
class PosConfig(models.Model):
    ...
    pos_customer_prefix = fields.Char(
        string='Customer ID Prefix',
        help='Short code shown before the customer number on receipts. '
             'e.g. enter KDTY for Kondotty shop → customer shows KDTY-00023. '
             'e.g. enter CHLR for Chelari shop → customer shows CHLR-00011. '
             'Leave blank to auto-derive from the POS name.',
    )