...
patch(PosOrder.prototype, {
    ...
    /* ================= HELPER: get MRP from product proxy ================= */
    _getMrpFromProduct(product) {
        if (!product) return 0;
        try {
            const rawSym = Object.getOwnPropertySymbols(product).find(s => s.toString().includes('raw'));
            if (!rawSym) return 0;
            return product[rawSym]?.mrp_price || 0;
        } catch (_) { return 0; }
    },
