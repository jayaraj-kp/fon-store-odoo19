...
# #         }
# # new code for negative amount display in the balance sheet  Liabilities & Equity
# # -*- coding: utf-8 -*-
# from odoo import models, fields, api
# import logging
#
...
#     # debit-minus-credit storage).  We flip the sign before display so
#     # the Balance Sheet always shows positive numbers for L & E.
#     # ------------------------------------------------------------------
#     CREDIT_NORMAL_TYPES = {
#         'liability_payable',
#         'liability_current',
...
#             # Odoo stores balance as (debit - credit), so these accounts carry
#             # a negative value internally.  Flip the sign so the Balance Sheet
#             # always displays positive figures for L & E sections.
#             # ----------------------------------------------------------------
#             if account_type in self.CREDIT_NORMAL_TYPES:
#                 balance      = -balance
#                 comp_balance = -comp_balance
#
#             if balance == 0.0 and comp_balance == 0.0:
...
#                 'code':         str(code or ''),
#                 'name':         str(name or ''),
#                 # Keep raw debit/credit for the D/C columns (no sign flip needed)
#                 'debit':        bal.get('debit',  0.0),
#                 'credit':       bal.get('credit', 0.0),
...
#             'total_equity':                  total_equity,
#             'comp_total_equity':             comp_total_equity,
#             # For the Balance Sheet to balance: Assets = Liabilities + Equity
#             # Both sides should equal the same positive number
#             'total_liabilities_equity':      total_liab + total_equity,
#             'comp_total_liabilities_equity': comp_total_liab + comp_total_equity,
#         }
...
class BalanceSheetInlineReport(models.TransientModel):
    ...
    # P&L account types used to compute Current Year Earnings manually.
    # Odoo 19 CE uses these internal type strings.
    INCOME_TYPES = (
        'income',
        'income_other',
        'other_income',
    ...
    # ------------------------------------------------------------------
    # Compute Current Year Earnings (Net Profit) from P&L accounts.
    #
    # In Odoo CE, account 999999 (current_year_earnings) is a VIRTUAL
    # account — it has no real entries in account_move_line.  Odoo
    # computes it on-the-fly as: Total Income - Total Expenses.
    # We replicate that logic here so the Balance Sheet balances.
    # ------------------------------------------------------------------
    def _compute_current_year_earnings(self, date_from=None, date_to=None):
        cr     = self.env.cr
        params = [self.company_id.id]
        ...
        pl_types = self.INCOME_TYPES + self.EXPENSE_TYPES
        params_with_types = params + [pl_types]
        ...
        for acc_type, bal in cr.fetchall():
            bal = float(bal)
            if acc_type in self.INCOME_TYPES:
                # Income accounts: credit normal → Odoo stores as negative balance
                # Flip sign to get positive income figure
                income_total += -bal
            else:
                # Expense accounts: debit normal → Odoo stores as positive balance
                expense_total += bal

        net_profit = income_total - expense_total
        _logger.debug(
            'bak_balance_sheet: CYE income=%.2f expense=%.2f net=%.2f',
    ...
    def _get_account_groups(self):
        ...
        # NOTE: 'current_year_earnings' is intentionally included here but
        # will typically return zero from account_move_line (virtual account).
        # We handle it separately via _compute_current_year_earnings() below.
        SECTION_MAP = {
            'asset_cash':            ('assets',      'Bank and Cash Accounts'),
            'asset_receivable':      ('assets',      'Receivables'),
            'asset_current':         ('assets',      'Current Assets'),
        ...
        for acc_id, code, name, account_type in rows:
            ...
            # ----------------------------------------------------------------
            # Liability and Equity accounts have a CREDIT normal balance.
            # Odoo stores balance as (debit - credit), so these carry a
            # negative value internally.  Flip the sign so the Balance Sheet
            # always displays positive figures for L & E sections.
            # ----------------------------------------------------------------
            if account_type in self.CREDIT_NORMAL_TYPES:
                balance      = -balance
            ...
            sections[section].setdefault(subsection, []).append({
                ...
                'code':         str(code or ''),
                'name':         str(name or ''),
                # Keep raw debit/credit for the D/C columns (no sign flip)
                'debit':        bal.get('debit',  0.0),
                'credit':       bal.get('credit', 0.0),
                # Sign-corrected balance for display
                'balance':      balance,
        ...
        # ------------------------------------------------------------------
        # Inject Current Year Earnings (Net Profit) as a virtual equity row.
        #
        # Odoo CE never posts entries directly to account 999999 during the
        # fiscal year — it is computed on-the-fly as Income minus Expenses.
        # We calculate it here and inject it so the Balance Sheet balances:
        ...
        comp_cye = 0.0
        if self.enable_comparison and self.comparison_date_to:
            comp_cye = self._compute_current_year_earnings(
                self.comparison_date_from, self.comparison_date_to)

        if cye != 0.0 or comp_cye != 0.0:
    ...
    def get_report_data(self):
        ...
        return {
            ...
            'total_equity':                  total_equity,
            'comp_total_equity':             comp_total_equity,
            # Assets = Liabilities + Equity  (both sides should match)
            'total_liabilities_equity':      total_liab + total_equity,
            'comp_total_liabilities_equity': comp_total_liab + comp_total_equity,
        }