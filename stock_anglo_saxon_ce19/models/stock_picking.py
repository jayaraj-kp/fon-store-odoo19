...
class StockPicking(models.Model):
    ...
    # DR  Stock Valuation          (226000)  ← value comes back to inventory
    # CR  Stock Interim (Delivered)(365000)  ← reverses/clears the interim
    # ─────────────────────────────────────────────────────────────────────────
    def _create_return_valuation_entry(self):
        """
        Create the REVERSED valuation entry for a customer return/refund picking.

        This is the exact opposite of _create_delivery_valuation_entry():
            Sale:   DR Stock Interim  /  CR Stock Valuation
            Return: DR Stock Valuation /  CR Stock Interim   ← this method

        Called by:
          - PosOrder._create_anglo_saxon_pos_entries() for POS refunds
          - StockPickingPos._action_done() for non-POS returns
        Uses receipt_journal_entry_ids as the guard/link field so it does
        not conflict with delivery_journal_entry_ids on the same picking.
        """
        self.ensure_one()
        line_vals = []
        ...
        for stock_move in self.move_ids.filtered(lambda m: m.state == 'done'):
            ...
            if not valuation_account:
                _logger.warning(
                    "Anglo-Saxon (Return): Stock Valuation Account missing on "
                    "category '%s' — skipping '%s'.", categ.name, product.name,
                )
                continue
            ...
            if not output_account:
                _logger.warning(
                    "Anglo-Saxon (Return): Stock Output Account missing on "
                    "category '%s' — skipping '%s'.", categ.name, product.name,
                )
                continue
            ...
            if value <= 0.0:
                _logger.debug(
                    "Anglo-Saxon (Return): zero value for '%s' — skipping.",
                    product.name,
                )
                continue
            ...
            _logger.info(
                "Anglo-Saxon (Return): picking='%s' product='%s' qty=%s "
                "unit_cost=%s value=%s  DR=%s (valuation)  CR=%s (interim)",
                self.name, product.name, qty, unit_cost, value,
                valuation_account.code, output_account.code,
            )

            line_vals += [
                {   # DR Stock Valuation — inventory value comes BACK
                    'name': desc,
                    'account_id': valuation_account.id,
                    'debit': value,
                    'credit': 0.0,
                ...
                {   # CR Stock Interim (Delivered) — reverses/clears interim
                    'name': desc,
                    'account_id': output_account.id,
                    'debit': 0.0,
                    'credit': value,
        ...
        if not line_vals:
            _logger.info("Anglo-Saxon (Return): no lines to post for '%s'.", self.name)
            return

        journal = self._get_stock_journal()
        if not journal:
            _logger.warning(
                "Anglo-Saxon (Return): No stock journal found for picking '%s'.",
                self.name,
            )
            return
        ...
        entry = self.env['account.move'].sudo().create({
            ...
            'journal_id': journal.id,
            'date': self.date_done or fields.Date.context_today(self),
            'ref': _('Stock Return Valuation: %s') % self.name,
            'line_ids': [(0, 0, v) for v in line_vals],
            'company_id': self.company_id.id,
        })
        entry.sudo().action_post()
        # Store in receipt_journal_entry_ids (used as guard for returns)
        self.sudo().receipt_journal_entry_ids = [(4, entry.id)]
        _logger.info(
            "Anglo-Saxon (Return): Created entry '%s' for picking '%s'.",
            entry.name, self.name,
        )
