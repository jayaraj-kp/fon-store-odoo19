...
export class SpecialOfferPopup extends Component {
    ...
    /**
     * Offers are already filtered by warehouse on the server side.
     * We only further filter by offer_type here.
     */
    get allFlatOffers() {
    ...
    lineMatchesOffer(line, offer) {
        ...
        // No fallback to "match all" — if no scope configured, match nothing.
        return false;
    }

    /**
     * FIXED: For 'fixed' discount type, we SUBTRACT the discount value
    ...
    /**
     * For FLAT DISCOUNT offers: apply to all matching lines.
     * For COUPON offers: apply only to matching lines (couponMode=true).
     */
    ...
    async applyCoupon() {
        ...
        // Apply the discount (coupon mode)
