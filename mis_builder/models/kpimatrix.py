...
class KpiMatrix:
    ...
    def iter_rows(self):
        ...
        FIX: Pre-load account names to avoid KeyError when accounts are deleted.
        Filters out rows with deleted accounts.
        """
        for kpi_row in self._kpi_rows.values():
            yield kpi_row
            detail_rows = self._detail_rows[kpi_row.kpi].values()
            # Load account names first to ensure cache is populated
            self._load_account_names()
            # Filter out rows with deleted accounts
            # Only keep detail rows where the account still exists
            detail_rows = [r for r in detail_rows if r.account_id in self._account_names]
            detail_rows = sorted(detail_rows, key=lambda r: r.label)
            yield from detail_rows
    ...
    def _load_account_names(self):
        """Load account names from database.

        FIX: Only loads accounts that still exist in the database.
        Accounts that have been deleted will not be added to the cache.
        """
        account_ids = set()
        for detail_rows in self._detail_rows.values():
            account_ids.update(detail_rows.keys())
    ...
    def get_account_name(self, account_id):
        ...
        FIX: Handles missing accounts gracefully by returning a placeholder
        instead of raising KeyError. This prevents crashes when accounts
        have been deleted but are still referenced in report data.

        Args:
            account_id: The ID of the account to retrieve the name for

        Returns:
            str: The account name, or a placeholder if account is deleted
        """
        if account_id not in self._account_names:
            self._load_account_names()

        # Handle missing accounts gracefully
        # (accounts that were deleted after being referenced in reports)
        if account_id not in self._account_names:
            _logger.warning(
                "Account %s referenced in MIS report but not found in database. "
