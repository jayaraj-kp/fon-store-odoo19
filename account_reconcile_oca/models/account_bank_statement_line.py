...
class AccountBankStatementLine(models.Model):
    ...
    def save(self):
        if self.env.context.get("reconcile_inline_create"):
            return False
        return {"type": "ir.actions.act_window_close"}

    def action_reconcile_inline_save_and_new(self):
        self.ensure_one()
        return {
            "type": "ir.actions.client",
            "tag": "account_reconcile_oca.reconcile_inline_create_done",
            "params": {"mode": "new"},
        }

    def action_reconcile_inline_save_and_close(self):
        self.ensure_one()
        return {
            "type": "ir.actions.client",
            "tag": "account_reconcile_oca.reconcile_inline_create_done",
