...
def _is_pos_generated_move(move):
    """
    Return True if this account.move was generated by POS.
    POS moves must NOT be touched by account_move.py — they are handled
    exclusively by pos_order.py to avoid double-stamping with the wrong
    (user's) warehouse analytic instead of the POS config's warehouse analytic.

    Detection: journal name prefix, move name prefix, or pos_session_id
    presence on a linked statement line.
    """
    # POS session closing moves: POSS/
    # POS order moves: POSJ/ or linked to a POS order
    name = move.name or ''
    if name.startswith('POSS/') or name.startswith('POSJ/'):
        return True

    # Invoice created from POS (action_pos_order_invoice) — has a pos_order ref
    # These are out_invoice but linked to POS; check via pos_order_ids
    if hasattr(move, 'pos_order_ids') and move.pos_order_ids:
        return True

    # Journal entries whose journal is a POS payment journal
    # (Card CHLR, Cash KDTY etc.) — these are bank statement line moves
    journal = move.journal_id
    if journal:
        # If any POS payment method uses this journal, it's a POS move
        pm_count = move.env['pos.payment.method'].search_count([
...
class AccountMove(models.Model):
    ...
    def _apply_warehouse_analytic_to_lines(self):
        """
        Stamp warehouse analytic (from the logged-in user's default warehouse)
        on ALL journal entry lines of invoices and bills.

        IMPORTANT: POS-generated moves are intentionally skipped here.
        They are handled by pos_order.py using the POS config's warehouse,
        which is the correct warehouse regardless of who is logged in.
        """
        analytic_account = self._get_warehouse_analytic_account()
        if not analytic_account:
            return
        ...
        for move in self:
            ...
            # Skip POS-generated moves — pos_order.py handles them
            if _is_pos_generated_move(move):
                _logger.debug(
                    'Skipping POS-generated move %s in account_move analytic stamping',
                    move.name,
                )
                continue

            # Target ALL lines including receivable/payable/tax.
            # display_type is 'line_section' or 'line_note' only for decorative rows.
            ...
            for line in lines_to_tag:
                ...
                if key not in existing:
                    ...
                    try:
                        ...
                        _logger.debug(
                            'Warehouse analytic %s applied to %s line %s (%s)',
                        ...
                        _logger.warning(
                            'Could not apply analytic to line %s on move %s: %s',
                            line.id, move.name, e,
                        )
    ...
    def action_post(self):
        # Apply BEFORE posting so lines are not yet locked
        self._apply_warehouse_analytic_to_lines()
        result = super().action_post()
        # Apply AFTER posting to catch receivable/payable lines Odoo creates
        # during finalization
        self._apply_warehouse_analytic_to_lines()
        return result
...
class AccountMoveLine(models.Model):
    ...
    def _onchange_product_apply_warehouse_analytic(self):
        if not self.move_id or self.move_id.move_type not in (
            ...
            return
        if _is_pos_generated_move(self.move_id):
            return
        stock_account = self._get_stock_input_account()
        if stock_account and self.account_id != stock_account:
            self.account_id = stock_account
