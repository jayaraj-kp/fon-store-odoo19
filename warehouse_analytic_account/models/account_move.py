...
class AccountMove(models.Model):
    ...
    def _apply_warehouse_analytic_to_lines(self):
        """
        Stamp warehouse analytic on ALL journal entry lines including:
        - Invoice/bill product lines
        - Receivable / Payable lines
        - Tax lines
        - Any other journal items

        Uses sudo() + check_move_validity=False so it works even on
        posted/locked moves (e.g. after action_post completes).
        """
        analytic_account = self._get_warehouse_analytic_account()
        if not analytic_account:
            return
        ...
        for move in self:
            ...
            # Target ALL lines — including receivable, payable, tax lines.
            # display_type is only 'line_section' or 'line_note' for decorative rows.
            # Receivable/payable lines have display_type = False — do NOT exclude them.
            ...
            for line in lines_to_tag:
                ...
                if key not in existing:
                    ...
                    new_dist[key] = 100.0
                    try:
                        # sudo() + bypass validity check to write on posted/locked moves
    ...
    def action_post(self):
        ...
        # Apply AFTER posting as well to catch the auto-generated
        # receivable / payable lines that Odoo finalizes during action_post
        self._apply_warehouse_analytic_to_lines()
        return result
