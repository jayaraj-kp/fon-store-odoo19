...
def _is_pos_move(move):
    ...
    POS moves must be skipped here — they are handled by pos_order.py
    using the POS config's warehouse (not the user's default warehouse).

    Detection strategies (any one is sufficient):
      A. Move name starts with POSS/ or POSJ/
      B. Move has pos_order_ids
      C. Move's journal is used by any POS payment method
    """
    name = move.name or ''
    if name.startswith(('POSS/', 'POSJ/')):
    ...
    for line in lines_to_tag:
        ...
        if key not in existing:
            ...
            try:
                ...
                _logger.debug(
                    'Analytic %s → %s line %s (%s)',
                    analytic_account.name,
                    label or move.name or 'draft',
                    line.id,
                    line.account_id.code,
                )
            except Exception as e:
...
class AccountMove(models.Model):
    ...
    def _apply_warehouse_analytic_to_lines(self):
        ...
        Payment entries (move_type='entry') are handled by AccountPayment below.
        POS moves are handled by pos_order.py.
        """
        analytic_account = self._get_warehouse_analytic_account()
        if not analytic_account:
            return
    ...
    def action_post(self):
        self._apply_warehouse_analytic_to_lines()
        result = super().action_post()
        # Second pass: catch receivable/payable lines added during posting
        self._apply_warehouse_analytic_to_lines()
        return result
...
class AccountPayment(models.Model):
    ...
    def action_post(self):
        """
        After posting the payment, stamp the warehouse analytic on all lines
        of the resulting journal entry (move_type='entry').

        This covers ALL payment scenarios:
        - Manual payments registered directly on a payment form
        - Payments created via the 'Register Payment' wizard from an invoice
        - Payments created programmatically

        Why here and not in AccountMove?
        AccountMove._apply_warehouse_analytic_to_lines() intentionally skips
        move_type='entry' moves to avoid tagging unrelated journal entries
        (depreciation, accruals, bank charges, etc.). Every payment — regardless
        of how it was initiated — goes through account.payment.action_post(),
        making this the single clean, safe hook for payment moves.

        POS payments are excluded — their journal belongs to a POS payment
        method which is detected by _is_pos_move().
        """
        result = super().action_post()
        ...
        for payment in self:
            ...
            if _is_pos_move(move):
                _logger.debug(
                    'AccountPayment.action_post: skipping POS payment move %s',
                    move.name,
                )
                continue
            _logger.debug(
                'AccountPayment.action_post: applying analytic %s to %s',
