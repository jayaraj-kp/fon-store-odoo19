...
class ReportCustomLabel(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        show_mrp_price = self._resolve_show_mrp(data)
        _logger.info(
            "BARCODE LABEL: label_qty=%s show_mrp_price=%s",
            label_qty, show_mrp_price,
        )
        return {
            'doc_ids': docids,
    ...
    def _resolve_qty(self, docids, data):
        ...
        try:
            ...
            if parsed:
                ...
                _logger.warning(
                    "BARCODE: config_param had no matching ids. stored=%s docids=%s",
                    stored, docids,
                )
        except Exception as e:
    ...
    @api.model
    def _resolve_show_mrp(self, data):
        """
        Resolve show_mrp_price with 2 fallback layers:
          1. data dict  — passed directly by the controller via the data= argument
          2. ir.config_parameter — set by the wizard before the dialog opens
        Returns True if MRP Price should be shown, False for Sales Price.
        """
        # Layer 1: data dict (primary path — set by controller)
        try:
            if data and 'show_mrp_price' in data:
                val = data['show_mrp_price']
                result = bool(val) and str(val) not in ('0', 'false', 'False', '')
                _logger.info("BARCODE: show_mrp from data dict: %s", result)
                return result
        except Exception as e:
            _logger.warning("BARCODE: show_mrp data dict error: %s", e)

        # Layer 2: ir.config_parameter (fallback — set by wizard)
        try:
            raw = self.env['ir.config_parameter'].sudo().get_param(
                'custom_barcode_label.pending_show_mrp', '0'
            )
            result = (raw == '1')
            _logger.info("BARCODE: show_mrp from config_param: %s (raw=%s)", result, raw)
            return result
        except Exception as e:
            _logger.warning("BARCODE: show_mrp config_param error: %s", e)
...
class ReportCustomLabelTmpl(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        show_mrp_price = self._resolve_show_mrp(data)
        _logger.info(
            "BARCODE TMPL: label_qty=%s show_mrp_price=%s",
            label_qty, show_mrp_price,
        )
        return {
            'doc_ids': docids,
