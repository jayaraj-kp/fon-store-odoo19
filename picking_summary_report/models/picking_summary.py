...
class StockPickingSummary(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # ------------------------------------------------------------------
        # Group moves by (category, source_location, product, uom)
        # Each group collects one entry per destination with its qty + ref
        # ------------------------------------------------------------------
        # key   : (category_name, source_location, product_name, uom_name)
        # value : list of {'destination': ..., 'qty': ..., 'picking_ref': ...}
        # ------------------------------------------------------------------
        grouped = defaultdict(list)
        order_key = {}   # keeps first-seen sort order for stable output
        ...
        for picking in pickings:
            for move in picking.move_ids:
                ...
                if key not in order_key:
                    # store a sort tuple so we can sort later
                    order_key[key] = (
                        category.complete_name or '',
                        product.name or '',
                        picking.location_id.display_name or '',
                    )
                grouped[key].append({
        ...
        # Sort groups by category → product → location
        sorted_keys = sorted(grouped.keys(), key=lambda k: order_key[k])

        summary_lines = []
        for serial, key in enumerate(sorted_keys, start=1):
            category_name, source_location, product_name, uom_name = key
            destinations = grouped[key]   # list of {destination, qty, picking_ref}
            ...
            summary_lines.append({
                ...
                'uom':             uom_name,
                # Each entry: {'destination': ..., 'qty': ..., 'picking_ref': ...}
                'destinations':    destinations,
            })

        picking_refs = ', '.join(pickings.mapped('name'))
        warehouses   = list(set(pickings.mapped('picking_type_id.warehouse_id.name')))
