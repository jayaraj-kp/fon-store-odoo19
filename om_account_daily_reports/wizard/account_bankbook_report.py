...
class AccountBankBookReport(models.TransientModel):
    ...
    def action_preview_report(self):
        ...
        records = report_obj.with_context(
        ...
        return {
            ...
            'name': _('Bank Book – Preview'),
            'res_model': 'account.bankbook.report',
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref(
                'om_account_daily_reports.view_bankbook_preview_form'
            ).id,
            'target': 'new',
    ...
    def _entry_link(self, move_name, label):
        if not move_name:
            return label
        # Search for the move by name to get its id
    ...
    def _build_preview_html(self, records, data, codes):
        ...
        sortby = form.get('sortby', 'sort_date')
        sort_lbl = 'Date' if sortby == 'sort_date' else 'Journal & Partner'
...
<style>
  ...
  .dr-table td.r { text-align: right; font-variant-numeric: tabular-nums;
                   white-space: nowrap; }
  .dr-acct > td { background: #dde6f5; font-weight: bold;
                  border-top: 2px solid #b0c4de;
                  border-bottom: 1px solid #afc1de; }
  .dr-acct > td.r { color: #1a3a6b; }
  .dr-line > td { background: #fff; }
  .dr-line:hover > td { background: #f5f8ff; }
  .dr-init > td { background: #fef9e7; font-weight: 600; font-style: italic;
                  border-bottom: 1px dashed #d4ac0d; color: #7d6608; }
  .dr-init > td.r { text-align: right; }
...
"""]
        ...
        parts.append(
            f'<div class="dr-meta-item">'
            f'<strong>Sorted By</strong><span>{sort_lbl}</span></div>'
        )
        parts.append(
            f'<div class="dr-meta-item">'
        ...
        if not records:
            ...
            for account in records:
                a_code = account.get('code', '')
                a_name = account.get('name', '')
                a_debit = account.get('debit', 0.0)
                a_credit = account.get('credit', 0.0)
                a_balance = account.get('balance', 0.0)
                grand_debit += a_debit
                grand_credit += a_credit
                grand_balance += a_balance

                parts.append(
                    f'<tr class="dr-acct">'
                    f'<td colspan="6">{a_code} {a_name}</td>'
                    f'<td class="r">{money(a_debit)}</td>'
                    f'<td class="r">{money(a_credit)}</td>'
                    f'<td class="r">{money(a_balance)}</td>'
                    f'</tr>'
                )
