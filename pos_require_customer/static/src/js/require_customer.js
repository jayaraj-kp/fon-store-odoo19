 ...
 * Entry points covered:
 *   ✅ "Payment" button           → pos.pay()            (runtime wrap in setup)
 *   ✅ "Cash KDTY" button         → selectPaymentMethod() (prototype patch)
 *   ✅ "Card KDTY" button         → selectPaymentMethod() (prototype patch)
 *   ✅ Payment method buttons     → openPaymentInterfaceElectronically() (patch)
 *   ✅ Validate (safety net)      → validateOrder()       (PaymentScreen patch)
 *   ✅ Fast validate              → validateOrderFast()   (PaymentScreen patch)
 */
...
patch(ProductScreen.prototype, {
    ...
    // ── Patch selectPaymentMethod for Cash KDTY / Card KDTY buttons ─
    ...
    // ── Patch clickPaymentMethod (alternative entry point) ─
    async clickPaymentMethod(paymentMethod) {
        if (noCustomer(this.pos)) {
            await showPopup(
                this._rcDialog,
    ...
    // ── Patch openPaymentInterfaceElectronically for card payments ─
    ...
    // ── Patch selectCashPaymentMethod for cash button ─
...
patch(PaymentScreen.prototype, {
    ...
    // ── Additional safety patches for PaymentScreen methods ─
    async selectPaymentMethod(paymentMethod) {
        if (noCustomer(this.pos)) {
            await showPopup(
                this._rcDialog,
