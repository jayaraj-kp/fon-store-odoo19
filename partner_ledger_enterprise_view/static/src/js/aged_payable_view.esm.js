///** @odoo-module **/
///**
// * Aged Payable Report — Enterprise Interactive View
// */
//
//import {
//    Component,
...
//}
//
//export class AgedPayableView extends Component {
//    static template = "pl.AgedPayableView";
//    static props = ["*"];
//
//    setup() {
...
//            analyticAccountsAll: [],
//            analyticAccountsLoaded: false,
//            analyticSearchTerm: "",
//            analyticDropdownOpen: false,
//
...
//        this.analyticDropdownRef = useRef("analyticDropdownRef");
...
//            ["analyticDropdownOpen", this.analyticDropdownRef],
...
//        } catch (e) {
//            console.error("Aged Payable init error:", e);
//        }
//    }
//
//    async _loadData() {
...
//                analytic_account_ids: this.state.selectedAnalyticAccounts.map((a) => a.id),
...
//        } catch (e) {
//            console.error("Aged Payable data error:", e);
//            this.state.error =
...
//    }
//
//    get analyticFilterLabel() {
//        const count = this.state.selectedAnalyticAccounts.length;
//        return count ? `Analytic (${count})` : "Analytic";
//    }
//
//    get filteredAnalyticAccounts() {
//        const term = (this.state.analyticSearchTerm || "").toLowerCase().trim();
//        if (!term) return this.state.analyticAccountsAll;
//        return this.state.analyticAccountsAll.filter((a) => {
...
//            "analyticDropdownOpen",
//            "optionsDropdownOpen",
//            "intervalDropdownOpen",
//            "basedOnDropdownOpen",
//        ].forEach((k) => (this.state[k] = false));
//        this.state[key] = !wasOpen;
//        if (name === "analytic" && this.state[key] && !this.state.analyticAccountsLoaded) {
//            this._loadAnalyticAccountsList();
//        }
//    }
//
//    // ------------------------------------------------------------------
...
//    }
//
//    async _loadAnalyticAccountsList() {
//        try {
//            const results = await rpc("/pl/search_analytic_accounts", {
//                term: "",
//                limit: 500,
//            });
//            this.state.analyticAccountsAll = results || [];
//            this.state.analyticAccountsLoaded = true;
//        } catch (e) {
//            console.error("Analytic accounts load error:", e);
//        }
//    }
//
//    onAnalyticSearchInput(ev) {
//        this.state.analyticSearchTerm = ev.target.value;
//    }
//
//    toggleAnalyticAccount(account) {
//        const idx = this.state.selectedAnalyticAccounts.findIndex(
//            (a) => a.id === account.id
//        );
//        if (idx >= 0) {
//            this.state.selectedAnalyticAccounts.splice(idx, 1);
//        } else {
//            this.state.selectedAnalyticAccounts.push(account);
//        }
//        this._loadData();
//    }
//
//    isAnalyticAccountChecked(accountId) {
//        return this.state.selectedAnalyticAccounts.some((a) => a.id === accountId);
//    }
//
//    // ------------------------------------------------------------------
...
//                link.download = result.file_name || "aged_payable.pdf";
...
//                link.download = result.file_name || "aged_payable.xlsx";
//                document.body.appendChild(link);
//                link.click();
//                document.body.removeChild(link);
//            }
...
//}
//
//registry.category("actions").add("pl.AgedPayableView", AgedPayableView);
...
export class AgedPayableView extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            // Lazy-load control: stay on a placeholder screen (pick
            // partners / "Load All Partners") until data has actually
            // been requested at least once, instead of auto-querying
            // every partner on open.
            dataLoaded: false,
        ...
        onWillStart(async () => {
            await this._loadInitData();
            // Only auto-load when the report was opened with a specific
            // partner/tag/analytic scope already applied. A plain fresh
            // open waits for the user to pick partners or press "Load
            // All Partners" below, since querying every partner by
            // default is what causes the lag on large databases.
            if (
                this.state.selectedPartners.length ||
                this.state.selectedTags.length ||
                this.state.selectedAnalyticAccounts.length
            ) {
                await this._loadData();
            }
        });
    ...
    /** "Load All Partners" button on the initial placeholder screen —
     * loads the report with whatever filters are currently set (normally
     * none, i.e. every partner) without requiring a search first. */
    loadAllData() {
        this._loadData();
    }
