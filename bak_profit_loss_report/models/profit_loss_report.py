...
class ProfitLossReport(models.TransientModel):
    ...
    def _get_pl_groups(self):
        ...
        SECTION_MAP = {
            'income':                  ('revenue',      'Sales Revenue'),
            'income_other':            ('other_income', 'Other Income'),
            'expense_direct_cost':     ('cogs',         'Cost of Revenue'),
            'expense':                 ('opex',         'General & Admin Expenses'),
            'expense_depreciation':    ('opex',         'Depreciation & Amortisation'),
        }
    ...
    def get_report_data(self):
        ...
        cogs_subs,  total_cogs,         comp_cogs         = _build(sections['cogs'])
        opex_subs,  total_opex,         comp_opex         = _build(sections['opex'])
        oinc_subs,  total_other_income, comp_other_income = _build(sections['other_income'])

        gross_profit        = total_revenue - total_cogs
        comp_gross_profit   = comp_revenue  - comp_cogs

        operating_income      = gross_profit      - total_opex
        comp_operating_income = comp_gross_profit - comp_opex

        # Other Income/Expense sit BELOW Operating Income, not inside Gross Profit
        net_profit      = operating_income      + total_other_income
        comp_net_profit = comp_operating_income + comp_other_income
        ...
        return {
            ...
            'comp_operating_income':      comp_operating_income,
            # Other Income (non-operating, e.g. cash difference gains, interest)
            'other_income':               oinc_subs,
            'total_other_income':         total_other_income,
            'comp_total_other_income':    comp_other_income,
            # Net Profit = Operating Income + Other Income
            'net_profit':                 net_profit,
            'comp_net_profit':            comp_net_profit,
        }