...
<script src="{js_url}"></script>
    ...
    def _build_pdf_html(self, data):
        ...
        summary_row('OPERATING INCOME (OR LOSS)',
                    data.get('operating_income', 0), data.get('comp_operating_income', 0),
                    'gross-profit-row')

        section_html('Other Income', data.get('other_income', []),
                     data.get('total_other_income', 0), data.get('comp_total_other_income', 0))
        body += f'<tr><td colspan="{ncols}" style="height:4px"></td></tr>'
...
<table><thead><tr>{col_headers}</tr></thead><tbody>{body}</tbody></table>
    ...
    def profit_loss_xlsx(self, wizard_id, **kwargs):
        ...
        c = 2
        if show_dc:
            ws.write(row, c, 'Debit', hdr_fmt);
            c += 1
            ws.write(row, c, 'Credit', hdr_fmt);
            c += 1
        ws.write(row, c, 'Amount', hdr_fmt);
        c += 1
        if show_comp:
            ws.write(row, c, f'Comp ({data.get("comparison_date_to", "")})', hdr_fmt)
        row += 1
        ...
        def write_section(title, subs, total, comp_total):
            ...
            for sub in subs:
                ...
                for r in sub.get('rows', []):
                    ...
                    if show_dc:
                        ...
                        c += 1
                        ws.write(row, c, r.get('credit', 0), num_fmt);
                        c += 1
                    nf = neg_fmt if r['balance'] < 0 else num_fmt
                    ws.write(row, c, r['balance'], nf);
                    c += 1
                    if show_comp:
                        nf2 = neg_fmt if r['comp_balance'] < 0 else num_fmt
                        ws.write(row, c, r['comp_balance'], nf2)
                ...
                c = 2
                if show_dc: c += 2
                ws.write(row, c, sub['subtotal'], sub_fmt);
                c += 1
                if show_comp: ws.write(row, c, sub.get('comp_subtotal', 0), sub_fmt)
                row += 1
            # section total
            ...
            c = 2
            if show_dc: c += 2
            ws.write(row, c, total, sec_tot_fmt);
            c += 1
            if show_comp: ws.write(row, c, comp_total, sec_tot_fmt)
            row += 2
        ...
        write_section('Operating Expenses',
                      data.get('opex', []), data['total_opex'], data.get('comp_total_opex', 0))
        write_summary('OPERATING INCOME (OR LOSS)',
                      data.get('operating_income', 0), data.get('comp_operating_income', 0), gp_lbl_fmt, gp_num_fmt)
        write_section('Other Income',
                      data.get('other_income', []), data.get('total_other_income', 0),
                      data.get('comp_total_other_income', 0))
        write_summary('NET PROFIT',
                      data['net_profit'], data.get('comp_net_profit', 0), np_lbl_fmt, np_num_fmt)
    ...
    # ------------------------------------------------------------------
    @http.route('/bak/profit_loss/drilldown', type='http', auth='user')
    def profit_loss_drilldown(self, account_id, date_from=None, date_to=None, target_move='posted', analytic_ids=None,
                              **kwargs):
        env = request.env
        account_id = int(account_id)
        account = env['account.account'].browse(account_id)
