...
class ProductLabelWizard(models.TransientModel):
    ...
    def _build_html(self, label_list):
        """
        GP-1125T roll: 152mm wide, labels feed vertically (portrait).

        Each label fits on ONE physical sticker:
        ┌────────────────────────────────────────────────┐
        │ [QR] │  KEYCHAIN                               │
        │      │  KC110      MRP Rs. 300                 │
        └────────────────────────────────────────────────┘

        Layout: QR on left, text block on right — all in one row.
        2 labels per row, side by side on the roll.

        Physical sticker size (approximate): 65mm wide × 26mm tall
        """

        # ── Dimensions (all in mm) ──────────────────────────────────────────
        LW       = 65     # label width  (one sticker)
        LH       = 26     # label height (one sticker — fits in single cell)
        QR_SIZE  = 18     # QR image size (fits inside LH with small padding)
        COL_GAP  = 22     # gap between the 2 side-by-side labels
        ROW_GAP  = 2      # vertical gap between rows of labels
        L_MAR    = 5      # left margin on roll
        PW       = 160    # page/roll width mm
        ...
        # ── Build pages: 2 labels per row, 1 row per page ──────────────────
        page_h = LH + ROW_GAP + 4   # tight: one row per page + small margins
...
#                 # 'text-align:center;'
#                 '">'
#                 '<div style="font-size:20pt;font-weight:;'
#                 'text-transform:uppercase;white-space:nowrap;">'
#                 + (lbl['name'] or '') + '</div>'
#                 + mrp_html +
#                 '</td></tr>'
