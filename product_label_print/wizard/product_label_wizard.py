...
class ProductLabelWizard(models.TransientModel):
    ...
    def _get_label_rows(self):
        """
        Return label_list split into pairs (rows of 2) for the QWeb template.
        Each row is a list of 1 or 2 label dicts.
        """
        label_list = self._get_label_list()
        rows = []
        for i in range(0, len(label_list), 2):
            rows.append(label_list[i:i + 2])
        return rows

    # ── Print action — opens Odoo's native print dialog ──────────────────────
    ...
    def action_print_labels(self):
        ...
        # Trigger the QWeb PDF report — this opens Odoo's print preview dialog
        # (the same dialog shown in the screenshot with Print / Save as PDF buttons)
        return self.env.ref(
