...
class ProductLabelWizard(models.TransientModel):
    ...
    # JSON storage for label data — read by QWeb template via _parse_label_data()
    label_data_json = fields.Text(string='Label Data JSON', default='[]')

    def _parse_label_data(self):
        """Called from QWeb template to get the list of label dicts."""
        self.ensure_one()
        try:
    ...
    def action_print_labels(self):
        ...
        # Store on the transient record — QWeb reads it via _parse_label_data()
