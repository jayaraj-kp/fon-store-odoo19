...
/**
 * Compute round-up to the next multiple of 5.
 *
 * Examples (matching your requirement):
 *   994 → 1   (994 + 1 = 995)
 *   995 → 5   (995 + 5 = 1000)  ← 995 is itself a multiple of 5, so go to NEXT one
 *   996 → 4   (996 + 4 = 1000)
 *   997 → 3   (997 + 3 = 1000)
 *   998 → 2   (998 + 2 = 1000)
 *   999 → 1   (999 + 1 = 1000)
 *   993 → 2   (993 + 2 = 995)
 *   992 → 3   (992 + 3 = 995)
 *   1000→ 5   (1000 + 5 = 1005) ← already a multiple of 5, go to next
 *
 * Rule: always round UP to the NEXT multiple of 5 (never stay at current).
 */
function computeRoundOff(total) {
    const floored = Math.floor(total);           // ignore paise/decimals
    const rem     = floored % 5;                 // remainder when divided by 5
    const diff    = rem === 0 ? 5 : (5 - rem);  // if already multiple of 5 → 5, else gap to next
    return diff;
}
...
export class CharityOrderButton extends Component {
    ...
    /**
     * Round-up amount to next multiple of 5.
     * This is shown on the donate button label AND used as the one-tap amount.
     * e.g. total=997 → shows "Donate ₹3 (Round Up to ₹1000)"
     */
    get orderRoundOffAmount() {
        const order = this.currentOrder;
        if (!order) return 0;
    ...
    /**
     * The rounded-up total (what customer will pay).
     * e.g. 997 + 3 = 1000
     */
    get orderRoundedTotal() {
        const order = this.currentOrder;
        if (!order) return 0;
        const total = getOrderTotal(order);
        return Math.floor(total) + computeRoundOff(total);
    }
    ...
    async openOrderCharityPopup() {
        ...
        const result = await makeAwaitable(this.dialog, CharityDonationPopup, {
            title:          this.charityButtonLabel,
            changeAmount:   roundOff,   // cap = exactly the round-off amount
            roundOffAmount: roundOff,   // shown on green one-tap button
            ceilAmount:     roundOff,
            currencySymbol: this.currencySymbol,
        });
