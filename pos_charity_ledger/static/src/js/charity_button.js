...
export class CharityOrderButton extends Component {
    ...
    async openOrderCharityPopup() {
        ...
        if (result && result.confirmed && result.amount > 0) {
            ...
            // Add to the session-level running total displayed in the UI
            this.pos._sessionCharityTotal = parseFloat(
                ((this.pos._sessionCharityTotal || 0) + result.amount).toFixed(2)
            );

            this.notification.add(
                this.currencySymbol + result.amount.toFixed(2) +
                " will be donated to charity when register closes. Thank you!",
    ...
    removeOrderCharityDonation() {
        const order = this.currentOrder;
        if (order) {
            const prev = order._charity_donation_amount || 0;
            if (prev > 0 && this.pos._sessionCharityTotal) {
                this.pos._sessionCharityTotal = parseFloat(
                    Math.max(0, (this.pos._sessionCharityTotal || 0) - prev).toFixed(2)
                );
            }
...
patch(PaymentScreen.prototype, {
    ...
    _applyCharityDonation(amount) {
        ...
        // Track session total in UI
        this.pos._sessionCharityTotal = parseFloat(
            ((this.pos._sessionCharityTotal || 0) + amount).toFixed(2)
        );

        this.notification.add(
            this.currencySymbol + amount.toFixed(2) +
            " will be donated to charity when register closes. Thank you!",
            { type: "success", sticky: false }
        );
    },
