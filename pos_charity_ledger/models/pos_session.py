...
class PosSession(models.Model):
    ...
    def _compute_charity_totals(self):
        """
        Read totals from pos.charity.donation to avoid cross-model
        @depends issues at module load time.
        """
        for session in self:
            donations = self.env['pos.charity.donation'].search([
                ('pos_session_id', '=', session.id),
    ...
    # ------------------------------------------------------------------
    # NOTE: No _create_pending_charity_journal_entries needed.
    # With the product-line approach, the charity amount is already split
    # correctly in the standard POS invoice via the product's income account.
    # The session closing entry (POSS/...) and cash statement (CSKDT/...)
    # are created by Odoo core — we don't need to add anything.
    # ------------------------------------------------------------------
    ...
    def _loader_params_product_product(self):
        """
        Ensure the CHARITY_ROUNDOFF product is always loaded into the POS
        UI even if it is not in the configured pricelist or category filter,
        so charity_button.js can find it and add it as an order line.
        """
        ...
        if charity_product:
            existing_domain = result.get('search_params', {}).get('domain', [])
            if existing_domain:
                result['search_params']['domain'] = [
                    '|'
                ] + existing_domain + [
                    ('id', '=', charity_product.id)
                ]
            else:
                result['search_params']['domain'] = [
                    '|',
