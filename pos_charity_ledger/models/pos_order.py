...
class PosOrder(models.Model):
    ...
    def _process_order(self, order, existing_order):
        ...
        How the money flows (Scenario A — donation set on ORDER screen):
            Order total  = ₹467
            Cashier pays = ₹467 cash
            → amount_paid = 467, amount_return = 0
            Odoo check: 467 - 0 = 467 ✓
            charity_donation_amount = 3  (saved as a separate field)
            At session close: journal entry moves ₹3 cash → charity GL.

        How the money flows (Scenario B — donation set on PAYMENT screen):
            Order total  = ₹467
            Customer pays ₹470 cash
            → amount_paid = 470, amount_return = 3
            Cashier donates the ₹3 change.
            Odoo check: 470 - 3 = 467 ✓
            charity_donation_amount = 3  (saved as a separate field)
            At session close: journal entry moves ₹3 cash → charity GL.

        We deliberately do NOT touch amount_return in either scenario.
        """
    ...
    def _create_charity_donation(self):
        ...
        Journal entry is deferred to session close so it appears
        together with the other POS session accounting entries."""
        self.ensure_one()
        if not (self.charity_donation_amount > 0 and self.charity_account_id):
            return
        # Avoid duplicate donation records on re-validate
    ...
    def _create_charity_journal_entry(self):
        ...
        if not config.charity_gl_account_id:
            _logger.info(
                'No charity GL account configured, skipping journal entry for order %s',
                self.name,
            )
            return
        try:
            ...
            # Determine the debit account (cash/card account that received the donation)
            debit_account = False
            ...
            if not debit_account:
                _logger.error(
                    'Could not determine debit account for order %s', self.name
                )
                return
            ...
            _logger.info(
                'Charity journal entry %s created for order %s',
                move.name, self.name,
            )
        except Exception as e:
            _logger.error(
